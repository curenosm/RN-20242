...
begin
	slide_t = @bind t_0 Slider(20:0.0001:30, default=0.5);
	slide_y = @bind y_0 Slider(3:0.001:10, default=4.3);
	slide_delta = @bind delta Slider(0.0001:0.001:0.1, default=0.0001);
	slide_max_t = @bind max_t Slider(1:0.1:60, default=30);

	# Constantes para la forma general de la ecuación
	slide_A = @bind A Slider(-15:0.1:15, default=0);
	slide_B = @bind B Slider(-15:0.1:15, default=2.3);
	slide_C = @bind C Slider(-15:0.1:15, default=1);
	slide_D = @bind D Slider(-15:0.1:15, default=1);
	slide_F = @bind F Slider(-15:0.1:15, default=1);
	slide_H = @bind H Slider(-15:0.1:15, default=1);
	
	nothing
end

# ╔═╡ f33721d0-ecd1-4522-b3ee-abaef6f5b265
md"""
Tiempo Inicial: $(slide_t) \
Y Inicial: $(slide_y) \
Delta: $(slide_delta) \
Max T: $(slide_max_t) \
A: $(slide_A) \
...
# ╔═╡ c35401e4-874b-4d13-ba5c-5fb64eb59afd
function print_cur_values()
	println("t_0: $t_0")
	println("y_0: $y_0")
	println("delta: $delta")
	println("max_t: $max_t")
...
# ╔═╡ 8bc876e5-d0c3-4984-a89d-d4fb7cb82dbf
print_cur_values()
...
"""
function alpha_n(V, epsilon = 0.01)
	_alpha_n(V) = (A + B * V) / (C + H * exp((V + D) / F))
...
# ╔═╡ 9aeae710-f982-4363-98cc-377f0c9d7e33
print_cur_values()

# ╔═╡ 8c323089-c187-415d-a844-83f0f0d0c82b
alpha_n(4.7)

# ╔═╡ 5b44f3f8-d8f6-4a83-9bd3-141f14c699ff
alpha_n(1:0.001:10)
...
manifest_format = "2.0"
project_hash = "1a75f6c51cb6284548a1781216913435c0da3f79"
...
git-tree-sha1 = "6cf78165dd5b10c0b8df85ff3f1f69e25580ad3b"
